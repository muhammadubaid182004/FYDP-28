/**
 * Updated Image Classifier Component
 * Uses Python backend API instead of local ONNX inference
 */

import { useState, useRef } from "react";
import { useClassificationAPI } from "@/hooks/useClassificationAPI";
...
export function ImageClassifierAPI() {
  const [selectedImage, setSelectedImage] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(null);
  const [result, setResult] = useState<ClassificationResult | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { classify, loading, error } = useClassificationAPI();
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a fundus photograph for AI-powered classification (Backend Processing)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <p className="text-xs text-gray-500 text-center">
            ℹ️ Image is sent to backend service for processing
          </p>
        </CardContent>
      </Card>
        ...
        <Card className={`border-2 ${getSeverityColor(result.severity)}`}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div
                className={`inline-block px-4 py-2 rounded-full font-semibold ${getSeverityBadgeColor(
                  result.severity
                )}`}
              >
                {result.severity}
            ...
            <Alert>
              <AlertDescription className="text-xs">
                ⚠️ This is an AI-assisted analysis tool. Results should be reviewed by qualified
                ophthalmologists. Not for diagnostic use without professional review.
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
